...
public class Encrypts {
     ...
     */
    public static String encryptAES(final String data,
                                    final String key,
                                    final String transformation,
                                    final String iv) {
        String encryptStr = "";
        try {
            byte[] encryptBytes = symmetricTemplate(data.getBytes("UTF-8"), key.getBytes("UTF-8"), "AES", transformation, iv.getBytes("UTF-8"), true);
            encryptStr = new String(encryptBytes, Charset.forName("UTF-8"));
     ...
     * @return the bytes of AES decryption
     */
    public static String decryptAES(final String data,
                                    final String key,
                                    final String transformation,
                                    final String iv) {
        String encryptStr = "";
        try {
            byte[] encryptBytes = symmetricTemplate(data.getBytes("UTF-8"), key.getBytes("UTF-8"), "AES", transformation, iv.getBytes("UTF-8"), false);
            encryptStr = new String(encryptBytes, Charset.forName("UTF-8"));
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        } catch (NullPointerException e) {
    ...
    public static byte[] base64Encode(final byte[] input, int flags) {
        return Base64.encode(input, flags);
    }

    public static byte[] base64Decode(final byte[] input, int flags) {
