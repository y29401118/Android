...
public class MessagesStorage extends BaseController {
        ...
        storageQueue.postRunnable(() -> {
            try {
                ...
                while (filtersCursor.next()) {
                    ...
                    MessagesController.DialogFilter oldfilter = dialogFiltersMap.get(filter.id);
                    if(oldfilter != null){
                        dialogFiltersMap.delete(filter.id);
                        dialogFilters.remove(oldfilter);
                    }
    ...
    private void saveDialogFilterInternal(MessagesController.DialogFilter filter, boolean atBegin, boolean peers) {
        try {
            MessagesController.DialogFilter existfilter = dialogFiltersMap.get(filter.id);
            if (existfilter == null) {
                if (atBegin) {
    ...
    private synchronized void updateFiltersReadCounter(LongSparseArray<Integer> dialogsToUpdate, LongSparseArray<Integer> dialogsToUpdateMentions, boolean read) throws Exception {
    ...
    private synchronized void updateDialogsWithReadMessagesInternal(final ArrayList<Integer> messages, final SparseLongArray inbox, SparseLongArray outbox, final ArrayList<Long> mentions) {
        ...
        try {
            ...
            if (dialogsToUpdate.size() > 0 || dialogsToUpdateMentions.size() > 0) {
                ...
                if (dialogsToUpdate.size() > 0) {
                    ...
                    for (int a = 0; a < dialogsToUpdate.size(); a++) {
                        ...
                        messageException = "prevUnreadCount:"+prevUnreadCount+" newCount:"+newCount;
                        state.requery();
            ...
            FileLog.e(e);
            CrashReport.postCatchedException(new Throwable(e.getMessage()+messageException));
        }
    }
    ...
    private synchronized void putUsersInternal(ArrayList<TLRPC.User> users) throws Exception {
        ...
        for (int a = 0; a < users.size(); a++) {
            ...
            try {
                ...
                state.dispose();
                state = database.executeFast("REPLACE INTO users VALUES(?, ?, ?, ?)");
                state.requery();
            }
    ...
    private synchronized void putChatsInternal(ArrayList<TLRPC.Chat> chats) throws Exception {
    ...
    private synchronized void updateRepliesMaxReadIdInternal(int chatId, int mid, int readMaxId) {
        try {
            SQLitePreparedStatement state = database.executeFast("UPDATE messages SET replies_data = ? WHERE mid = ?");
    ...
    public synchronized void updateRepliesCount(int chatId, int mid, ArrayList<TLRPC.Peer> repliers, int maxId, int count) {
        storageQueue.postRunnable(() -> {
            try {
                SQLitePreparedStatement state = database.executeFast("UPDATE messages SET replies_data = ? WHERE mid = ?");
                long messageId = mid | ((long) chatId) << 32;
    ...
    public synchronized void updateMessageVerifyFlags(ArrayList<TLRPC.Message> messages) {
    ...
    private synchronized void putMessagesInternal(final ArrayList<TLRPC.Message> messages, final boolean withTransaction, final boolean doNotUpdateDialogDate, final int downloadMask, boolean ifNoLastMessage, boolean scheduled) {
       String messageException = "";
        try {
            if (scheduled) {
                if (withTransaction) {
                ...
                for (int a = 0; a < messages.size(); a++) {
                    TLRPC.Message message = messages.get(a);
                    fixUnsupportedMedia(message);
                    messageException = "message"+message.message+" id:"+message.id;
                    state_messages.requery();
                    long messageId = message.id;
                    if (message.local_id != 0) {
                        messageId = message.local_id;
    ...
    private synchronized long[] updateMessageStateAndIdInternal(long random_id, Long _oldId, int newId, int date, int channelId, int scheduled) {
        SQLiteCursor cursor = null;
        long oldMessageId;
    ...
    private synchronized void updateUsersInternal(final ArrayList<TLRPC.User> users, final boolean onlyStatus, final boolean withTransaction) {
        try {
            if (onlyStatus) {
                if (withTransaction) {
                    database.beginTransaction();
    ...
    private synchronized void markMessagesAsReadInternal(SparseLongArray inbox, SparseLongArray outbox, SparseIntArray encryptedMessages) {
        try {
            if (!isEmpty(inbox)) {
    ...
    protected synchronized void deletePushMessages(long dialogId, final ArrayList<Integer> messages) {
        try {
    ...
    private synchronized ArrayList<Long> markMessagesAsDeletedInternal(final ArrayList<Integer> messages, int channelId, boolean deleteFiles, boolean scheduled) {
        try {
    ...
    private synchronized void updateDialogsWithDeletedMessagesInternal(final ArrayList<Integer> messages, ArrayList<Long> additionalDialogsToUpdate, int channelId) {
        try {
            String ids;
            ArrayList<Long> dialogsToUpdate = new ArrayList<>();
            if (!messages.isEmpty()) {
    ...
    private synchronized ArrayList<Long> markMessagesAsDeletedInternal(final int channelId, final int mid, boolean deleteFiles) {
        try {
            String ids;
            ArrayList<Long> dialogsIds = new ArrayList<>();
            LongSparseArray<Integer[]> dialogsToUpdate = new LongSparseArray<>();
    ...
    private synchronized void doneHolesInTable(String table, long did, int max_id) throws Exception {
        if (max_id == 0) {
    ...
    public synchronized void doneHolesInMedia(long did, int max_id, int type) throws Exception {
        if (type == -1) {
            if (max_id == 0) {
    ...
    public synchronized void closeHolesInMedia(long did, int minId, int maxId, int type) {
        try {
            boolean ok = false;
            SQLiteCursor cursor;
            if (type < 0) {
    ...
    private synchronized void closeHolesInTable(String table, long did, int minId, int maxId) {
        try {
            boolean ok = false;
    ...
    public synchronized void putMessages(final TLRPC.messages_Messages messages, final long dialog_id, final int load_type, final int max_id, final boolean createDialog, final boolean scheduled) {
        storageQueue.postRunnable(() -> {
            try {
                if (scheduled) {
    ...
    private synchronized void putDialogsInternal(final TLRPC.messages_Dialogs dialogs, int check) {
        try {
            database.beginTransaction();
    ...
    private synchronized void checkIfFolderEmptyInternal(int folderId) {
        try {
