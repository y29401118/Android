...
public class MessagesController extends BaseController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public void cleanup() {
        NotificationBadge.applyCount(0);
    ...
    public void loadFullUser(final TLRPC.User user, final int classGuid, boolean force) {
        ...
        int reqId = getConnectionsManager().sendRequest(req, (response, error) -> {
            if (error == null) {
                ...
                if(getContactsController().contactsDict.get(userFull.user.id) == null
                && userFull.user.contact){
//                    TLRPC.TL_contact contact = new TLRPC.TL_contact();
//                    contact.user_id = userFull.user.id;
//                    getContactsController().contactsDict.put(userFull.user.id,contact);
                    getContactsController().addContact(userFull.user,false);
                }else if(getContactsController().contactsDict.get(userFull.user.id) != null
                        && !userFull.user.contact){
//                    getContactsController().contactsDict.remove(userFull.user.id);
//                    getContactsController().contacts.remove(contact);
                    ArrayList<TLRPC.User> users = new ArrayList<>();
                    users.add(userFull.user);
                    getContactsController().deleteContact(users);
                }
                AndroidUtilities.runOnUIThread(() -> {
        ...
        if (photo == null) {
            ...
            getConnectionsManager().sendRequest(req, (response, error) -> {
                if (error == null) {
                    ...
                    users.add(user1);
                    getMessagesStorage().putUsersAndChats(users, null, false, true);
                    if (photos_photo.photo instanceof TLRPC.TL_photo && !photos_photo.photo.sizes.isEmpty()) {
    ...
    public void loadSignUpNotificationsSettings() {
        if (!loadingNotificationSignUpSettings) {
            ...
            getConnectionsManager().sendRequest(req, (response, error) -> AndroidUtilities.runOnUIThread(() -> {
                ...
                SharedPreferences.Editor editor = notificationsPreferences.edit();
                enableJoined = response instanceof TLRPC.TL_boolTrue;
    ...
    public void addUserToChat(final int chat_id, final TLRPC.User user, int count_fwd, String botHash, final BaseFragment fragment, final Runnable onFinishRunnable) {
        ...
        getConnectionsManager().sendRequest(request, (response, error) -> {
            if (request instanceof  TLRPC.TL_messages_addChatUser) {
    ...
    /**
     *
     * @param pts 针对 非加密消息的自增序号（关联用户或者频道自增）
     * @param date
     * @param qts  针对 非加密消息的自增序号 （关联用户或者频道自增）
     * @param slice
     */
            ...
            if (response != null) {
                ...
                getMessagesStorage().getStorageQueue().postRunnable(() -> AndroidUtilities.runOnUIThread(() -> {
                    ...
                    for (int a = 0; a < dialogs.size(); a++) {
                        ...
                        if ((int) dialog.id == 0 && dialog.pinned) {
    ...
    public void processUpdates(final TLRPC.Updates updates, boolean fromQueue) {
        ...
        } else if (updates instanceof TLRPC.TL_updatesCombined || updates instanceof TLRPC.TL_updates)
        {
            ...
            for (int a = 0; a < updates.chats.size(); a++) {
                TLRPC.Chat chat = updates.chats.get(a);
                TLRPC.Dialog dialog = dialogs_dict.get(-(long) chat.id);
                if (ChatObject.isNotInChat(chat) && dialog != null && (promoDialog == null || promoDialog.id != dialog.id)) {
