    ...
    private void loadOtherLanguages() {
        ...
        if (!TextUtils.isEmpty(locales)) {
            ...
            for (String locale : localesArr) {
                ...
                // 临时解决方案，多了一个中文语言，加载显示过滤
                if (localeInfo != null && !"zh_hans_raw".equalsIgnoreCase(localeInfo.baseLangCode)) {
        ...
        try {
            ...
            if (dateDay == day && year == dateYear) {
               // return LocaleController.formatString("LastSeenFormatted", R.string.LastSeenFormatted, LocaleController.formatString("TodayAtFormatted", R.string.TodayAtFormatted, getInstance().formatterDay.format(new Date(date))));
                int diff = (int) (ConnectionsManager.getInstance(UserConfig.selectedAccount).getCurrentTime() - temDate) / 60;
                if (diff < 1) {
                    return LocaleController.getString("LastSeenNow", R.string.LastSeenNow);
                } else if (diff < 60) {
                    return LocaleController.formatString("LastSeenMinutes", R.string.LastSeenMinutes,diff+"");
                } else {
                    return LocaleController.formatString("LastSeenHours", R.string.LastSeenHours,(int) Math.ceil(diff / 60.0f)+"");
                }
            } else if (dateDay + 1 == day && year == dateYear) {
            ...
            if (response != null) {
                AndroidUtilities.runOnUIThread(() -> {
                    ...
                    for (int a = 0, size = res.objects.size(); a < size; a++) {
                        ...
                        if (existing == null) {
                            if(!"zh_hans_raw".equalsIgnoreCase(localeInfo.baseLangCode)){
                                languages.add(localeInfo);
                                languagesDict.put(localeInfo.getKey(), localeInfo);
                            }
                        } else {
