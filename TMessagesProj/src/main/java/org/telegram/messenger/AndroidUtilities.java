...
public class AndroidUtilities {
    ...
    private static File getAlbumDir(boolean secretChat) {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            storageDir = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES), "Wegrame");
        ...
        if (f != null && f.exists()) {
            if (parentFragment != null && f.getName().toLowerCase().endsWith("attheme")) {
                ...
                try {
                    ...
                    if (Build.VERSION.SDK_INT >= 24) {
                        intent.setDataAndType(FileProvider.getUriForFile(activity, BuildConfig.APPLICATION_ID + ".fileProvider", f), realMimeType != null ? realMimeType : "text/plain");
                    } else {
                    ...
                    if (realMimeType != null) {
                        try {
                            ...
                            if (Build.VERSION.SDK_INT >= 24) {
                                intent.setDataAndType(FileProvider.getUriForFile(activity, BuildConfig.APPLICATION_ID + ".fileProvider", f), "text/plain");
                            } else {
