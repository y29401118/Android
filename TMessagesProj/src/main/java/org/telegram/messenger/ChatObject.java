...
public class ChatObject {
    ...
    public static boolean isAllowAddFriend(TLRPC.Chat chat){
        if(chat == null){
            return true;
        }
        if(chat.creator || chat.admin_rights != null){
            return true;
        }
        if(chat.default_banned_rights != null && chat.default_banned_rights.embed_links){
            return false;
        }
        return true;
    }

    public static boolean hasAdminRight(TLRPC.ChatFull chat,TLRPC.User user){
        if(user == null || chat == null){
            return false;
        }
        ArrayList<TLRPC.ChatParticipant> participants = chat.participants.participants;
        if(participants == null || participants.size()<=0){
            return false;
        }
        for(TLRPC.ChatParticipant part: participants){
            if (part instanceof TLRPC.TL_chatChannelParticipant) {
                ...
                if (channelParticipant instanceof TLRPC.TL_channelParticipantCreator || channelParticipant instanceof TLRPC.TL_channelParticipantAdmin) {
                    ...
                    || part instanceof TLRPC.TL_chatParticipantAdmin) {
    ...
    public static boolean hasAdminRight(TLRPC.ChatParticipant part){
        if(part == null){
            return false;
        }
        if (part instanceof TLRPC.TL_chatChannelParticipant) {
