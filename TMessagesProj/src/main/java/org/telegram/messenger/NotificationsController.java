...
public class NotificationsController extends BaseController {
    ...
    public static String OTHER_NOTIFICATIONS_CHANNEL = "wegrame";
    ...
    private void scheduleNotificationRepeat() {
        try {
            ...
            if (minutes > 0 && personal_count > 0) {
                if (Build.VERSION.SDK_INT>=26) {
                    alarmManager.setExactAndAllowWhileIdle(AlarmManager.ELAPSED_REALTIME_WAKEUP, SystemClock.elapsedRealtime() + minutes * 60 * 1000, pintent);//这里就能让闹钟跟pendingIntent进行绑定，闹钟来临时就会广播Intent中的Action
                } else {
                    alarmManager.setExact(AlarmManager.ELAPSED_REALTIME_WAKEUP, SystemClock.elapsedRealtime() + minutes * 60 * 1000, pintent);
                }
            } else {
        ...
        if (!groupsCreated) {
            try {
                ...
                for (int a = 0; a < count; a++) {
                    NotificationChannel channel = list.get(a);
                    String id = channel.getId();
                    if (id != null && id.startsWith(keyStart)) {
...
//        isInApp = true;
        ...
        String channelId = preferences.getString(key, null);//1channel_groups_ia_-1193298821740528617
        String settings = preferences.getString(key + "_s", null);//dbbf40090f72a14ad120da4455fa271d
        boolean edited = false;
        ...
        if (channelId != null) {
            ...
            if (existingChannel != null) {
                if (!isSilent) {
                    int channelImportance = existingChannel.getImportance();//3
        ...
        return channelId;//1channel_groups_ia_-1193298821740528617
    }
                    ...
                    if (Build.VERSION.SDK_INT >= 26) {
                        ...
                        if (soundPath.equals(defaultPath)) {
                            ...
                            if (Build.VERSION.SDK_INT >= 24 && soundPath.startsWith("file://") && !AndroidUtilities.isInternalUri(Uri.parse(soundPath))) {
                                try {
                                    Uri uri = FileProvider.getUriForFile(ApplicationLoader.applicationContext, BuildConfig.APPLICATION_ID + ".fileProvider", new File(soundPath.replace("file://", "")));
                ...
                if (lowerId != 0) {
                    ...
                    if (preview[0] && Build.VERSION.SDK_INT >= Build.VERSION_CODES.P && !((ActivityManager) ApplicationLoader.applicationContext.getSystemService(Context.ACTIVITY_SERVICE)).isLowRamDevice()) {
                        if (!waitingForPasscode && !messageObject.isSecretMedia() && (messageObject.type == 1 || messageObject.isSticker())) {
                            ...
                            if (attach.exists()) {
                                try {
                                    uri = FileProvider.getUriForFile(ApplicationLoader.applicationContext, BuildConfig.APPLICATION_ID + ".fileProvider", attach);
                                } catch (Exception e) {
                                    FileLog.e(e);
                                    uri = null;
                                }
                    ...
                    if (preview[0] && !waitingForPasscode && messageObject.isVoice()) {
                        ...
                        if (!messages.isEmpty()) {
                            ...
                            if (Build.VERSION.SDK_INT >= 24) {
                                try {
                                    uri = FileProvider.getUriForFile(ApplicationLoader.applicationContext, BuildConfig.APPLICATION_ID + ".fileProvider", f);
