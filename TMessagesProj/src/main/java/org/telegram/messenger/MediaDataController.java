...
import org.telegram.utils.ShortcutPermission;
...
public class MediaDataController extends BaseController {
    ...
    private void toggleStickerSetInternal(Context context, int toggle, BaseFragment baseFragment, boolean showSettings, TLObject stickerSetObject, TLRPC.StickerSet stickerSet, int type, boolean showTooltip) {
        ...
        if (toggle != 0) {
            ...
            getConnectionsManager().sendRequest(req, (response, error) -> AndroidUtilities.runOnUIThread(() -> {
                ...
                loadStickers(type, false, true, true);
    ...
    public void installShortcut(long did) {
        try {
            if(ShortcutPermission.check(ApplicationLoader.applicationContext) == ShortcutPermission.PERMISSION_DENIED){
                Toast.makeText(ApplicationLoader.applicationContext, LocaleController.getString("chooseMe", R.string.shortcut_denied), Toast.LENGTH_SHORT).show();
                return;
            }
