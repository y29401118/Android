...
void addDatacenterAddress(JNIEnv *env, jclass c, jint instanceNum, jint datacenterId, jstring ipAddress, jint port) {
    const char *valueStr = env->GetStringUTFChars(ipAddress, 0);

    ConnectionsManager::getInstance(instanceNum).addDatacenterAddress((uint32_t) datacenterId, std::string(valueStr), (uint32_t) port);
...
static JNINativeMethod ConnectionsManagerMethods[] = {
        ...
        {"native_addDatacenterAddress", "(IILjava/lang/String;I)V", (void *) addDatacenterAddress},
