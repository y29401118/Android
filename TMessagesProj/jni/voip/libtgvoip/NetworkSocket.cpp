...
void NetworkSocket::GenerateTCPO2States(unsigned char* buffer, TCPO2State* recvState, TCPO2State* sendState){
	...
	uint32_t first1 = 0x44414548U, first2 = 0x54534f50U, first3 = 0x20544547U, first4 = 0x20544547U, first5 = 0xbbbbbbbbU;
	uint32_t second1 = 0;
	do {
	...
	} while (*first == first1 || *first == first2 || *first == first3 || *first == first4 || *first == first5 || *second == second1 || *reinterpret_cast<unsigned char*>(nonce) == 0xfe);

	// prepare encryption key/iv
	// memcpy(sendState->key, nonce + 8, 32);
	// memcpy(sendState->iv, nonce + 8 + 32, 16);
	memcpy(sendState->key, nonce + 16, 32);
	memcpy(sendState->iv, nonce + 16 + 32, 16);

	// prepare decryption key/iv
	char reversed[48];
	// memcpy(reversed, nonce + 8, sizeof(reversed));
	memcpy(reversed, nonce + 16, sizeof(reversed));
