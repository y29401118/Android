...
// std::string ConnectionsManager::socketHost = "159.138.148.78";
std::string ConnectionsManager::socketHost = "";
...
int ConnectionsManager::callEvents(int64_t now) {
    ...
    int32_t timeToPushPing = (int32_t) ((sendingPushPing ? 30000 : nextPingTimeOffset) -
...
void ConnectionsManager::select() {
    ...
    if (lastPauseTime != 0 && llabs(now - lastPauseTime) >= nextSleepTimeout) {
        ...
        if (!dontSleep) {
            for (requestsIter iter = runningRequests.begin();
...
void ConnectionsManager::onConnectionClosed(Connection *connection, int reason) {
    ...
    if (connection->getConnectionType() == ConnectionTypeGeneric) {
        ...
        scheduleTask([&, connection] {
            for (std::vector<std::unique_ptr<ProxyCheckInfo>>::iterator iter = proxyActiveChecks.begin();
                ...
                if (proxyCheckInfo->connectionNum == connection->getConnectionNum()) {
                    bool found = false;
                    for (requestsIter iter2 = runningRequests.begin();
...
void ConnectionsManager::processServerResponse(TLObject *message, int64_t messageId,
    ...
    if (typeInfo == typeid(TL_new_session_created)) {
        ...
        if (!connection->isSessionProcessed(response->unique_id)) {
            ...
            std::unique_ptr<TL_future_salt> salt = std::unique_ptr<TL_future_salt>(
                    new TL_future_salt());
                            ...
                            if (error->error_code == 500 || error->error_code < 0) {
                                ...
                                if (isWorkerBusy) {
                                    ...
                                    request->minStartTime = request->startTime +
                                                            (request->serverFailureCount > 10 ? 10
...
void ConnectionsManager::initDatacenters() {
    ...
    std::string _nebulaChatServer(ConnectionsManager::socketHost); //47.243.129.211
    // std::string _nebulaChatServer("192.168.0.113");
    // std::string _nebulaChatServer("192.168.1.70");
    ...
    if (!testBackend) {
        if (datacenters.find(1) == datacenters.end()) {
            ...
            datacenter->addAddressAndPort(ConnectionsManager::socketHost, _nebulaChatPort, 0, "");
        ...
        if (datacenters.find(2) == datacenters.end()) {
            ...
            datacenter->addAddressAndPort(ConnectionsManager::socketHost, _nebulaChatPort, 0, ""); // 47.242.16.109
...
void ConnectionsManager::detachConnection(ConnectionSocket *connection) {
    std::vector<ConnectionSocket *>::iterator iter = std::find(activeConnections.begin(),
                                                               activeConnections.end(), connection);
...
int32_t ConnectionsManager::sendRequest(TLObject *object, onCompleteFunc onComplete,
    ...
    scheduleTask(
            [&, requestToken, object, onComplete, onQuickAck, flags, datacenterId, connetionType, immediate] {
                Request *request = new Request(instanceNum, requestToken, connetionType, flags,
                                               datacenterId, onComplete, onQuickAck, nullptr);
                request->rawRequest = object;
...
void ConnectionsManager::sendRequest(TLObject *object, onCompleteFunc onComplete,
    ...
    scheduleTask(
            [&, requestToken, object, onComplete, onQuickAck, onWriteToSocket, flags, datacenterId, connetionType, immediate, ptr1, ptr2, ptr3] {
                ...
                Request *request = new Request(instanceNum, requestToken, connetionType, flags,
                                               datacenterId, onComplete, onQuickAck,
                                               onWriteToSocket);
                request->rawRequest = object;
                request->ptr1 = ptr1;
                request->ptr2 = ptr2;
...
void ConnectionsManager::removeRequestFromGuid(int32_t requestToken) {
    ...
    if (iter2 != guidsByRequests.end()) {
        ...
        if (iter != requestsByGuids.end()) {
            std::vector<int32_t>::iterator iter3 = std::find(iter->second.begin(),
        ...
        if (currentSize >= 3 * 1024 || a == count - 1) {
            ...
            if (transportData != nullptr) {
                if (reportAck && quickAckId != 0) {
                    ...
                    if (!requestIds.empty()) {
                        std::map<int32_t, std::vector<int32_t>>::iterator iter = quickAckIdToRequestIds.find(
                                quickAckId);
                        if (iter == quickAckIdToRequestIds.end()) {
...
void ConnectionsManager::requestSaltsForDatacenter(Datacenter *datacenter, bool media,
    ...
    request->num = 32;
    sendRequest(request, [&, datacenter, id, media](TLObject *response, TL_error *error,
                                                    int32_t networkType) {
                    std::vector<uint32_t>::iterator iter = std::find(requestingSaltsForDc.begin(),
                                                                     requestingSaltsForDc.end(), id);
                ...
                }, nullptr, RequestFlagWithoutLogin | RequestFlagEnableUnauthorized | RequestFlagUseUnboundKey,
...
inline void addMessageToDatacenter(uint32_t datacenterId, NetworkMessage *networkMessage,
    ...
    std::map<uint32_t, std::vector<std::unique_ptr<NetworkMessage>>>::iterator iter = messagesToDatacenters.find(
            datacenterId);
...
void ConnectionsManager::processRequestQueue(uint32_t connectionTypes, uint32_t dc) {
    ...
    for (requestsIter iter = runningRequests.begin(); iter != runningRequests.end();) {
        ...
        switch (request->connectionType & 0x0000ffff) {
            ...
            case ConnectionTypeDownload: {
                ...
                std::map<uint32_t, uint32_t>::iterator dcIter = downloadRunningRequestCount.find(
                        datacenterId);
    ...
    for (requestsIter iter = requestsQueue.begin(); iter != requestsQueue.end();) {
        ...
        if (request->requestFlags & RequestFlagTryDifferentDc) {
            ...
            if (requestStartTime != 0 && abs(currentTime - requestStartTime) >= timeout) {
                ...
                for (std::map<uint32_t, Datacenter *>::iterator iter2 = datacenters.begin();
                     iter2 != datacenters.end(); iter2++) {
    ...
    for (std::map<uint32_t, Datacenter *>::iterator iter = datacenters.begin();
        ...
        std::map<uint32_t, std::vector<std::unique_ptr<NetworkMessage>>>::iterator iter2 = genericMessagesToDatacenters.find(
                datacenter->getDatacenterId());
    ...
    for (std::map<uint32_t, std::vector<std::unique_ptr<NetworkMessage>>>::iterator iter = genericMessagesToDatacenters.begin();
         iter != genericMessagesToDatacenters.end(); iter++) {
                ...
                if (networkMessage->invokeAfter) {
                    ...
                    if (lastSentMessageRpcId != 0 && lastSentMessageRpcId != message->msg_id) {
                        ...
                        if (message->outgoingBody != nullptr) {
                            if (LOGS_ENABLED)
                                DEBUG_D("wrap outgoingBody(%p, %s) to TL_invokeAfterMsg",
                            ...
                            if (LOGS_ENABLED)
                                DEBUG_D("wrap body(%p, %s) to TL_invokeAfterMsg",
    ...
    for (std::map<uint32_t, std::vector<std::unique_ptr<NetworkMessage>>>::iterator iter = tempMessagesToDatacenters.begin();
         iter != tempMessagesToDatacenters.end(); iter++) {
        Datacenter *datacenter = getDatacenterWithId(iter->first);
        if (datacenter != nullptr) {
            std::vector<std::unique_ptr<NetworkMessage>> &array = iter->second;
    ...
    for (std::map<uint32_t, std::vector<std::unique_ptr<NetworkMessage>>>::iterator iter = genericMediaMessagesToDatacenters.begin();
         iter != genericMediaMessagesToDatacenters.end(); iter++) {
        Datacenter *datacenter = getDatacenterWithId(iter->first);
        if (datacenter != nullptr) {
            std::vector<std::unique_ptr<NetworkMessage>> &array = iter->second;
            sendMessagesToConnectionWithConfirmation(array,
                    ...
                    if (response != nullptr) {
                        ...
                        struct DatacenterInfo {
                            ...
                            void addAddressAndPort(TL_dcOption *dcOption) {
                                ...
                                for (std::vector<TcpAddress>::iterator iter = addresses->begin();
                        ...
                        for (uint32_t a = 0; a < count; a++) {
                            ...
                            std::map<uint32_t, std::unique_ptr<DatacenterInfo>>::iterator iter = map.find(
                                    (uint32_t) dcOption->id);
                            DatacenterInfo *info;
                            if (iter == map.end()) {
                        ...
                        if (!map.empty()) {
                            for (std::map<uint32_t, std::unique_ptr<DatacenterInfo>>::iterator iter = map.begin();
                                 iter != map.end(); iter++) {
                                Datacenter *datacenter = getDatacenterWithId(iter->first);
...
void ConnectionsManager::addDatacenterAddress(uint32_t datacenterId, std::string ipAddress,
                                                uint32_t port) {
    scheduleTask([&, datacenterId, ipAddress, port] {
        Datacenter *datacenter = getDatacenterWithId(datacenterId);
        if (datacenter != nullptr) {
            datacenter->addAddressAndPort(ipAddress, port, 0, "");
            datacenter->resetAddressAndPortNum();
        }
    });
}
...
void ConnectionsManager::applyDnsConfig(NativeByteBuffer *buffer, std::string phone, int32_t date) {
    scheduleTask([&, buffer, phone, date] {
        ...
        if (config != nullptr && config->date <= currentDate && currentDate <= config->expires) {
            ...
            for (std::vector<std::unique_ptr<TL_accessPointRule>>::iterator iter = config->rules.begin();
                 iter != config->rules.end(); iter++) {
                ...
                if (datacenter != nullptr) {
                    std::vector<TcpAddress> addresses;
                    for (std::vector<std::unique_ptr<IpPort>>::iterator iter2 = rule->ips.begin();
                         iter2 != rule->ips.end(); iter2++) {
                        ...
                        if (typeInfo == typeid(TL_ipPort)) {
                            ...
                            addresses.push_back(TcpAddress(ipPort->ipv4, ipPort->port, 0,
                                                           std::string(
                                                                   (const char *) ipPort->secret->bytes,
...
void ConnectionsManager::checkProxyInternal(ProxyCheckInfo *proxyCheckInfo) {
    ...
    if (freeConnectionNum == -1) {
        ...
        ConnectionType connectionType = (ConnectionType) (ConnectionTypeProxy |
                                                          (freeConnectionNum << 16));
        Datacenter *datacenter = getDatacenterWithId(DEFAULT_DATACENTER_ID);
